...
public class GroupService {
    ...
    public GroupService(GroupChatMapper groupChatMapper, GroupMemberMapper groupMemberMapper,
                        ConversationMapper conversationMapper, UserMapper userMapper,
                        IdGenerator idGenerator, RabbitTemplate rabbitTemplate) {
    ...
    private void sendSystemMessage(String conversationId, int convType, String content, String subType, String msgType) {
        try {
            ...
            rabbitTemplate.convertAndSend(exchange, "system.message", event);
        } catch (Exception e) {
            log.warn("发送系统消息失败: {}", e.getMessage());
        }
    }

    private void ensureConversation(String convId, int type) {
    ...
    public void updateGroupInfo(Long groupId, Long userId, Map<String, Object> body) {
        ...
        if (updatedName != null || updatedAvatar != null) {
            ...
            for (GroupMember m : members) {
                // TODO Phase 5: push session update via MQ to chat-service
            }
        }

        // 系统通知：只在值确实变化时发送
        if (updatedName != null && !updatedName.equals(group.getName())) {
            sendSystemMessage("g_" + groupId, 1, "群名称已修改为 \"" + updatedName + "\"", "name_change", "system");
        }
        ...
        if (updatedNotice != null && !updatedNotice.isEmpty() && !updatedNotice.equals(group.getNotice())) {
            sendSystemMessage("g_" + groupId, 1, updatedNotice, "announcement", "announcement");
        }
        ...
        if (added > 0) {
            ...
            for (Object raw : rawIds) {
                ...
                if (groupMemberMapper.findByGroupAndUser(groupId, uid) != null) {
                    // TODO Phase 5: init session via MQ to chat-service
                }
            }
        }
        for (Object raw : rawIds) {
            ...
            if (groupMemberMapper.findByGroupAndUser(groupId, uid) != null) {
                ...
                sendSystemMessage("g_" + groupId, 1, (u != null ? u.getNickname() : uid) + " 加入了群聊", "member_join", "system");
            }
        }
    ...
    public void removeMember(Long groupId, Long operatorId, Long targetUserId) {
        ...
        sendSystemMessage("g_" + groupId, 1, (removedUser != null ? removedUser.getNickname() : targetUserId) + " 被移出群聊", "member_leave", "system");
