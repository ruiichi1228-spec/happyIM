...
const selectSession = async (s) => {
  ...
  if (s.type == 1) {
    try { await groupCache.batchFetch([Number(s.peerId)]); const res = await request.get(`/groups/${s.peerId}`); if (res.code===0) { groupDetail.value=res.data; const gi = groupInfo[Number(s.peerId)]; if (gi) { groupDetail.value.name = gi.name; groupDetail.value.avatarUrl = gi.avatarUrl; groupDetail.value.memberCount = gi.memberCount } groupMembers.value=res.data.members||[]; editGroup.value={name:gi?.name||'',notice:res.data.notice||'',description:res.data.description||'',allowInvite:res.data.allowInvite!==false}; userCache.setAll(res.data.members); const me = res.data.members?.find(m => m.userId === myUserId.value); myGroupNickname.value = me?.groupNickname || '' } } catch(e){}
  } else {
...
const fetchSessions = async () => { try { const res = await request.get('/conversations'); if (res.code===0) { const list = res.data; updateUnread(list.reduce((s, x) => s + (x.unreadCount||0), 0)); const userIds = []; const groupIds = []; for (const s of list) { const parts = s.conversationId.substring(2).split('_'); if (s.type === 0) { const peer = Number(parts[0]) === myUserId.value ? Number(parts[1]) : Number(parts[0]); s.peerId = peer; userIds.push(peer) } else { const gid = Number(parts[0]); s.peerId = gid; groupIds.push(gid) } } await userCache.batchFetch(userIds); await groupCache.batchFetch(groupIds); for (const s of list) { if (s.type === 0) { const u = userCache.get(s.peerId); s.peerName = u?.remark || u?.nickname || u?.username || ''; s.peerAvatar = u?.avatarUrl || '' } else { const g = groupCache.get(s.peerId); s.peerName = g?.name || ''; s.peerAvatar = g?.avatarUrl || ''; s.memberCount = g?.memberCount || 0 } } sessions.value = [...list] } } catch(e) {} }
