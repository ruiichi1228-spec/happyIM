...
const fetchSessions = async () => { try { const res = await request.get('/conversations'); if (res.code===0) { const list = res.data; updateUnread(list.reduce((s, x) => s + (x.unreadCount||0), 0)); const userIds = list.filter(s => s.type === 0).map(s => Number(s.peerId)).filter(Boolean); const groupIds = list.filter(s => s.type === 1).map(s => Number(s.peerId)).filter(Boolean); await userCache.batchFetch(userIds); await groupCache.batchFetch(groupIds); // 用缓存补全缺失的会话名字 for (const s of list) { if (!s.peerName) { if (s.type === 0) { const u = userCache.get(Number(s.peerId)); if (u) { s.peerName = u.remark || u.nickname || u.username || ''; s.peerAvatar = u.avatarUrl || '' } } else { const g = groupCache.get(Number(s.peerId)); if (g) { s.peerName = g.name || ''; s.peerAvatar = g.avatarUrl || '' } } } } sessions.value = list } } catch(e) {} }
