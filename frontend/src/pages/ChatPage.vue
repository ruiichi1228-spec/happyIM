...
const fetchSessions = async () => { try { const res = await request.get('/conversations'); if (res.code===0) { const list = res.data; updateUnread(list.reduce((s, x) => s + (x.unreadCount||0), 0)); // 收集需要查缓存的 ID const userIds = []; const groupIds = []; for (const s of list) { const parts = s.conversationId.substring(2).split('_'); if (s.type === 0) { const peer = Number(parts[0]) === myUserId.value ? Number(parts[1]) : Number(parts[0]); s.peerId = peer; userIds.push(peer) } else { const gid = Number(parts[0]); s.peerId = gid; groupIds.push(gid) } } await userCache.batchFetch(userIds); await groupCache.batchFetch(groupIds); // 从 cache 填充会话信息 for (const s of list) { s.type = s.conversationId.startsWith('g_') ? 1 : 0; if (s.type === 0) { const u = userCache.get(s.peerId); s.peerName = u?.remark || u?.nickname || u?.username || ''; s.peerAvatar = u?.avatarUrl || '' } else { const g = groupCache.get(s.peerId); s.peerName = g?.name || ''; s.peerAvatar = g?.avatarUrl || ''; s.memberCount = g?.memberCount || 0 } } sessions.value = list } } catch(e) {} }
